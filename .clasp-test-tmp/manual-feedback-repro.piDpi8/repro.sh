...
JSON
  ...
  sleep 0.3
  content=''
  if [[ -f "$workspace_path" ]]; then content="$(cat "$workspace_path")"; fi
...
INNER
chmod +x "$root/codex"
mkdir -p "$root/workspace/.clasp-test-tmp" "$root/xdg-cache"
printf '%s\n' transient-noise > "$root/workspace/.clasp-test-tmp/noise.txt"
printf '%s\n' 'Make the feedback loop converge after verifier feedback.' > "$root/task.md"
CLASP_LOOP_CODEX_BIN_JSON='"'$root'/codex"' \
CLASP_LOOP_TASK_FILE_JSON='"'$root'/task.md"' \
CLASP_LOOP_WORKSPACE_JSON='"'$root'/workspace"' \
CLASP_LOOP_MAX_ATTEMPTS_JSON='2' \
XDG_CACHE_HOME="$root/xdg-cache" \
runtime/target/debug/claspc run examples/feedback-loop/Main.clasp -- "$root/state"
