...
run_verify() {
  cd "$project_root"
  "$claspc_bin" check examples/prompt-functions/Main.clasp
  "$claspc_bin" compile examples/prompt-functions/Main.clasp -o "$compiled_path"
  "$claspc_bin" native-image examples/prompt-functions/Main.clasp -o "$image_path"
  node examples/prompt-functions/demo.mjs "$compiled_path" "$image_path"
}
