...
`Weft` should be judged by whether it improves real software delivery by AI coding agents, not by whether it merely looks elegant on paper.

The core benchmark question is:

How much does using `Weft` improve end-to-end performance for existing agent harnesses such as `Codex` and `Claude Code` on realistic software tasks?

## Primary Thesis

The strongest public claim for `Weft` is not:

- "the syntax is shorter"

The stronger claim is:

- "agents using Weft finish more tasks correctly, with fewer interventions, fewer repair loops, lower total token cost, and stronger safety guarantees"
...
The practical question is not whether `Weft` is elegant in isolation. The practical question is whether an agent using a real harness performs better with `Weft` than with a baseline language.
...
`Weft` should be auditable at the benchmark layer too.
...
- The relative improvement of a given harness-model pair when using `Weft` versus a baseline language on the same task suite.
...
This is the clearest way to answer: "How much better does `Codex` or `Claude Code` perform when the project is written in Weft?"
...
This is especially important for `Weft`, because shared types and strong static semantics are part of the language value proposition.
...
This helps show whether `Weft` actually delivers on "one language everywhere."
...
The initial target is not to beat every language on every axis. The target is to prove that `Weft` produces a measurable harness-level advantage in the workflows it is designed for.
...
The benchmark program should shape `Weft`, not merely evaluate it after the fact.
...
## Target Claims for Weft

Over time, `Weft` should aim to demonstrate:
...
3. Mirror the same benchmark scenarios in early `Weft` as the language matures.
...
The point is to start measuring early, even before `Weft` is feature-complete.
