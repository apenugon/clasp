...
Native merge policies can now also carry task-local process and workspace-root allowlists through `claspc swarm policy set --allow-process ... --allow-workspace ...` and ordinary Clasp `policySetWithAccess`. Tool and verifier runs outside the executable allowlist or launched from a cwd outside the allowed workspace roots are rejected before spawn and audited as `process_permission_denied` or `workspace_permission_denied` events. This is the first native capability-policy slice; full filesystem operation mediation, network, and richer capability policies remain follow-on work.
