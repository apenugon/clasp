...
- Generated JavaScript modules also expose app-facing secret-consumer helpers on agent, route, tool, and workflow contracts plus reusable provider-binding adapters, so hosts resolve declared secret handles against an explicit boundary and provider instead of reaching into ambient process state.
