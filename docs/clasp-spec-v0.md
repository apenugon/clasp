...
- The Bun worker runtime also exposes a supervised `hotSwap` protocol that stages one bounded old/new module overlap, validates module and workflow compatibility against `__claspModule`, and delegates state migration or run activation through the generated workflow upgrade handlers before retiring the drained version.
