...
- The Bun worker runtime also exposes a supervised `hotSwap` protocol that stages one bounded old/new module overlap, validates module and workflow compatibility against `__claspModule`, supports operator handoff and draining before authority switches, and keeps an explicit rollback path live while the old version remains active.
