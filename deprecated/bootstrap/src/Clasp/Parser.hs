...
compactImportListParser = do
  keyword "with"
  sepEndBy1 compactImportParser (symbol ",")
...
agentRoleDeclParser = do
  ...
  _ <- symbol "="
  attrs <- agentRoleAttrParser `sepEndBy1` symbol ","
  end <- getSourcePos
  _ <- optional eol
  scn
...
guideDeclParser = do
  ...
  _ <- symbol "="
  entries <- braces (guideEntryDeclParser `sepEndBy` symbolN ",")
  end <- getSourcePos
  _ <- optional eol
  scn
  pure . TopGuideDecl $
...
mergeGateDeclParser = do
  ...
  (nameSpan, name) <- locatedLowerIdentifier
  _ <- symbol "="
  verifierRefs <- mergeGateVerifierRefParser `sepEndBy1` symbol ","
  end <- getSourcePos
  _ <- optional eol
  scn
...
recordDeclParser = do
  ...
  _ <- symbol "="
  fields <- braces (recordFieldDeclParser `sepEndBy` symbolN ",")
  end <- getSourcePos
  _ <- optional eol
  scn
  pure . TopRecordDecl $
...
workflowDeclParser = do
  ...
  (nameSpan, name) <- locatedUpperIdentifier
  _ <- symbol "="
  attrs <- braces (workflowAttrParser `sepEndBy` symbolN ",")
  end <- getSourcePos
  _ <- optional eol
  scn
...
supervisorDeclParser = do
  ...
  children <- braces (supervisorChildParser `sepEndBy` symbolN ",")
  end <- getSourcePos
  _ <- optional eol
  scn
...
policyDeclParser = do
  ...
  (nameSpan, name) <- locatedUpperIdentifier
  _ <- symbol "="
  classifications <- policyClassificationParser `sepEndBy1` symbolN ","
...
policyPermissionsParser = do
  keywordN "permits"
  braces (policyPermissionParser `sepEndBy` symbolN ",")
...
projectionDeclParser = do
  ...
  keyword "with"
  (policySpan, policyName) <- locatedUpperIdentifier
  fields <- braces (projectionFieldDeclParser `sepEndBy` symbolN ",")
  end <- getSourcePos
  _ <- optional eol
  scn
...
matchParser = do
  ...
  keyword "match"
  subject <- exprParser
  branches <- braces (matchBranchParser `sepEndBy1` symbolN ",")
  end <- getSourcePos
...
listExprParser = do
  start <- getSourcePos
  values <- brackets (exprParser `sepEndBy` symbolN ",")
  end <- getSourcePos
...
recordExprParser = do
  ...
  fields <- braces (recordFieldExprParser `sepEndBy` symbolN ",")
  end <- getSourcePos
