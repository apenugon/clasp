...
defaultFixHints code =
  case code of
    ...
    "E_BACKEND_TARGET_REQUIRES_NATIVE" ->
      ["Compile backend modules with `claspc native`, or split frontend/browser-only code into a separate module before using `claspc compile`."]
    "E_EMPTY_SUPERVISOR" ->
    ...
    "E_PRIMARY_COMPILER_SEED_MISSING" ->
      ["Regenerate `src/stage1.mjs`, or rerun with `--compiler=bootstrap` until the promoted hosted seed is restored."]
    "E_PRIMARY_COMPILER_NATIVE_SEED_MISSING" ->
      ["Regenerate `src/stage1.native.image.json`, or rerun with `--compiler=bootstrap` until the promoted hosted native seed is restored."]
