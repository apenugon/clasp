...
data CompileBundle = CompileBundle
  { compileBundleFrontendImplementation :: Maybe CompilerImplementation
  , compileBundleFrontendModule :: Maybe Text
  , compileBundleBackendImplementation :: Maybe CompilerImplementation
  , compileBundleBackendNativeIr :: Maybe Text
  , compileBundleBackendNativeImage :: Maybe Text
  }
  deriving (Eq, Show)
...
data HostedToolInput
  = HostedToolInputSource Text
  | HostedToolInputProjectEntry FilePath
  deriving (Eq, Show)
...
renderAirEntryJsonWithPreference preference entryPath =
  case preference of
    ...
    _ ->
      pure (Left unsupportedPrimaryCompilerArtifactDiagnostic)
...
renderContextEntryJsonWithPreference preference entryPath =
  case preference of
    CompilerPreferenceBootstrap -> do
      (_implementation, checkedModule) <- checkEntryWithPreference preference entryPath
...
renderNativeEntryWithPreference preference entryPath =
  case preference of
    ...
    CompilerPreferenceClasp ->
      runPrimaryTextTool HostedToolNative entryPath
    CompilerPreferenceAuto ->
      preferPrimaryTool
        alwaysTryPrimaryTool
        entryPath
        (const (runPrimaryTextTool HostedToolNative entryPath))
...
renderNativeImageEntryWithPreference preference entryPath =
  case preference of
    CompilerPreferenceBootstrap -> do
      ...
      pure (CompilerImplementationBootstrap, result)
    CompilerPreferenceClasp ->
      runPrimaryTextTool HostedToolNativeImage entryPath
    CompilerPreferenceAuto ->
      preferPrimaryTool
        alwaysTryPrimaryTool
        entryPath
        (const (runPrimaryTextTool HostedToolNativeImage entryPath))
...
checkEntrySummaryWithPreference preference entryPath =
  case preference of
    CompilerPreferenceBootstrap -> do
      ...
      pure (CompilerImplementationBootstrap, result)
    CompilerPreferenceClasp ->
      runPrimaryTextTool HostedToolCheck entryPath
    CompilerPreferenceAuto ->
      preferPrimaryTool
        alwaysTryPrimaryTool
        entryPath
        (const (runPrimaryTextTool HostedToolCheck entryPath))
...
compileEntryUncheckedWithPreference :: CompilerPreference -> FilePath -> IO (CompilerImplementation, Either DiagnosticBundle Text)
compileEntryUncheckedWithPreference preference entryPath =
  case preference of
    CompilerPreferenceBootstrap -> do
      result <- compileEntryBootstrap entryPath
      pure (CompilerImplementationBootstrap, result)
    CompilerPreferenceClasp ->
      runPrimaryTextTool HostedToolCompile entryPath
    CompilerPreferenceAuto ->
      preferPrimaryTool
        supportsPrimaryCompileAtPath
        entryPath
        (const (runPrimaryTextTool HostedToolCompile entryPath))
...
compileEntryWithPreference preference entryPath = do
  compileGuard <- ensureJavaScriptCompileAllowed entryPath
  case compileGuard of
    Just err ->
      pure (compileImplementationForPreference preference, Left err)
    Nothing ->
      compileEntryUncheckedWithPreference preference entryPath

compileBundleWithPreference :: CompilerPreference -> FilePath -> IO (Either DiagnosticBundle CompileBundle)
compileBundleWithPreference preference entryPath = do
  loadedModuleResult <- loadEntryModule entryPath
  case loadedModuleResult of
    Right modl
      | not (null (backendNativeSurfaceLabels modl)) -> do
          (frontendImplementation, frontendResult) <- compileEntryUncheckedWithPreference preference entryPath
          case frontendResult of
            Left err ->
              pure (Left err)
            Right frontendModule -> do
              (backendImplementation, nativeResult) <- renderNativeEntryWithPreference preference entryPath
              case nativeResult of
                Left err ->
                  pure (Left err)
                Right nativeIr -> do
                  (_imageImplementation, imageResult) <- renderNativeImageEntryWithPreference preference entryPath
                  case imageResult of
                    Left err ->
                      pure (Left err)
                    Right nativeImage ->
                      pure $
                        Right
                          CompileBundle
                            { compileBundleFrontendImplementation = Just frontendImplementation
                            , compileBundleFrontendModule = Just frontendModule
                            , compileBundleBackendImplementation = Just backendImplementation
                            , compileBundleBackendNativeIr = Just nativeIr
                            , compileBundleBackendNativeImage = Just nativeImage
                            }
    _ -> do
      (frontendImplementation, frontendResult) <- compileEntryWithPreference preference entryPath
      pure $
        case frontendResult of
          Left err ->
            Left err
          Right frontendModule ->
            Right
              CompileBundle
                { compileBundleFrontendImplementation = Just frontendImplementation
                , compileBundleFrontendModule = Just frontendModule
                , compileBundleBackendImplementation = Nothing
                , compileBundleBackendNativeIr = Nothing
                , compileBundleBackendNativeImage = Nothing
                }
...
explainEntryWithPreference preference entryPath =
  case preference of
    CompilerPreferenceBootstrap -> do
      ...
      pure (CompilerImplementationBootstrap, result)
    CompilerPreferenceClasp ->
      runPrimaryTextTool HostedToolExplain entryPath
    CompilerPreferenceAuto ->
      preferPrimaryTool
        alwaysTryPrimaryTool
        entryPath
        (const (runPrimaryTextTool HostedToolExplain entryPath))
...
runPrimaryCheckEntry entryPath = do
  let toolInput = HostedToolInputProjectEntry entryPath
  primaryResult <- runHostedTool HostedToolCheckCore toolInput Nothing
  case primaryResult of
    ...
    Right artifact -> do
      ...
      pure
        ( CompilerImplementationClasp
        , loadedModuleResult >>= \loadedModule -> decodeHostedCoreModule entryPath loadedModule artifact
        )
...
runPrimaryTextTool command entryPath = do
  let toolInput = HostedToolInputProjectEntry entryPath
  primaryResult <- runHostedTool command toolInput Nothing
...
alwaysTryPrimaryTool :: FilePath -> IO Bool
alwaysTryPrimaryTool _ = pure True

compileImplementationForPreference :: CompilerPreference -> CompilerImplementation
compileImplementationForPreference preference =
  case preference of
    CompilerPreferenceBootstrap ->
      CompilerImplementationBootstrap
    CompilerPreferenceClasp ->
...
ensureJavaScriptCompileAllowed :: FilePath -> IO (Maybe DiagnosticBundle)
ensureJavaScriptCompileAllowed entryPath = do
  loadedModuleResult <- loadEntryModule entryPath
  pure $
    case loadedModuleResult of
      Left _ ->
        Nothing
      Right modl ->
        let backendSurfaces = backendNativeSurfaceLabels modl
         in if null backendSurfaces
              then Nothing
              else Just (backendJavaScriptCompileUnsupportedDiagnostic backendSurfaces)

backendNativeSurfaceLabels :: Module -> [Text]
backendNativeSurfaceLabels modl =
  concat
    [ labelWhen "domain object" (not (null (moduleDomainObjectDecls modl)))
    , labelWhen "domain event" (not (null (moduleDomainEventDecls modl)))
    , labelWhen "feedback" (not (null (moduleFeedbackDecls modl)))
    , labelWhen "metric" (not (null (moduleMetricDecls modl)))
    , labelWhen "goal" (not (null (moduleGoalDecls modl)))
    , labelWhen "experiment" (not (null (moduleExperimentDecls modl)))
    , labelWhen "rollout" (not (null (moduleRolloutDecls modl)))
    , labelWhen "workflow" (not (null (moduleWorkflowDecls modl)))
    , labelWhen "supervisor" (not (null (moduleSupervisorDecls modl)))
    , labelWhen "guide" (not (null (moduleGuideDecls modl)))
    , labelWhen "hook" (not (null (moduleHookDecls modl)))
    , labelWhen "agent role" (not (null (moduleAgentRoleDecls modl)))
    , labelWhen "agent" (not (null (moduleAgentDecls modl)))
    , labelWhen "policy" (not (null (modulePolicyDecls modl)))
    , labelWhen "toolserver" (not (null (moduleToolServerDecls modl)))
    , labelWhen "tool" (not (null (moduleToolDecls modl)))
    , labelWhen "verifier" (not (null (moduleVerifierDecls modl)))
    , labelWhen "merge gate" (not (null (moduleMergeGateDecls modl)))
    , labelWhen "projection" (not (null (moduleProjectionDecls modl)))
    , labelWhen "route" (not (null (moduleRouteDecls modl)))
    ]
  where
    labelWhen label present =
      if present then [label] else []

backendJavaScriptCompileUnsupportedDiagnostic :: [Text] -> DiagnosticBundle
backendJavaScriptCompileUnsupportedDiagnostic backendSurfaces =
  singleDiagnostic
    "E_BACKEND_TARGET_REQUIRES_NATIVE"
    "Backend modules must be compiled with the native backend."
    [ "`claspc compile` emits JavaScript for frontend/browser modules only."
    , "Detected backend surfaces: " <> T.intercalate ", " backendSurfaces
    , "Use `claspc native` for this module."
    ]

supportsPrimaryCompileAtPath :: FilePath -> IO Bool
supportsPrimaryCompileAtPath entryPath = do
  loadedModuleResult <- loadEntryModule entryPath
  pure $
    case loadedModuleResult of
      Left _ ->
        True
      Right modl ->
        supportsHostedPrimaryCompileModule modl

supportsHostedPrimaryCompileModule :: Module -> Bool
supportsHostedPrimaryCompileModule modl =
  null (moduleDomainObjectDecls modl)
    && null (moduleDomainEventDecls modl)
    && null (moduleFeedbackDecls modl)
    && null (moduleMetricDecls modl)
    && null (moduleGoalDecls modl)
    && null (moduleExperimentDecls modl)
    && null (moduleRolloutDecls modl)
    && null (moduleWorkflowDecls modl)
    && null (moduleSupervisorDecls modl)
    && null (moduleGuideDecls modl)
    && null (moduleHookDecls modl)
    && null (moduleAgentRoleDecls modl)
    && null (moduleAgentDecls modl)
    && null (modulePolicyDecls modl)
    && null (moduleToolServerDecls modl)
    && null (moduleToolDecls modl)
    && null (moduleVerifierDecls modl)
    && null (moduleMergeGateDecls modl)
    && null (moduleProjectionDecls modl)
    && null (moduleForeignDecls modl)
    && null (moduleRouteDecls modl)

unsupportedPrimaryCompilerArtifactDiagnostic :: DiagnosticBundle
unsupportedPrimaryCompilerArtifactDiagnostic =
  singleDiagnostic
    "E_PRIMARY_COMPILER_UNSUPPORTED"
    "The primary Clasp compiler does not support this entrypoint yet."
    ["Use --compiler=bootstrap for this command while AIR and context generation stay on the recovery path."]
...
runHostedTool :: HostedToolCommand -> HostedToolInput -> Maybe Text -> IO (Either DiagnosticBundle Text)
runHostedTool command input expectedOutput = do
  nativeResult <- runHostedToolNative command input
  pure $
    case (nativeResult, expectedOutput) of
      (Left err, _) ->
        Left err
      (Right output, Nothing) ->
        Right output
      (Right output, Just expected)
        | output == expected ->
            Right output
        | otherwise ->
            Left $
              singleDiagnostic
                ...
                [ "hosted "
                    <> T.pack (renderHostedToolExportName command input)
                    <> " compatibility check did not reproduce the expected output"
                ]

runHostedToolNative :: HostedToolCommand -> HostedToolInput -> IO (Either DiagnosticBundle Text)
runHostedToolNative command input = do
        ...
        pure $
          Left $
            singleDiagnostic
              ...
              ["Regenerate src/stage1.native.image.json before running the primary Clasp compiler."]
      else do
        createDirectoryIfMissing True tempDir
        nativeInputArg <-
          case input of
            HostedToolInputSource hostedSource -> do
              let entrySourcePath = tempDir </> "entry.clasp"
              writeFileText entrySourcePath hostedSource
              pure entrySourcePath
            HostedToolInputProjectEntry projectEntryPath ->
              pure ("--project-entry=" <> projectEntryPath)
        (exitCode, _stdoutText, stderrText) <-
          readProcessWithExitCode
            "bash"
            [ hostedCompilerNativeToolRunnerPath
            , hostedCompilerNativeSeedPath
            , renderHostedToolExportName command input
            , nativeInputArg
            , primaryOutputPath
            ]
            ""
...
validateHostedCompileOutput output =
  let suspiciousPatterns =
        [ ("guide scope metadata", "scope:(")
        , ("guide plan metadata", "plan:,")
        , ("policy process metadata", "process \"")
        , ("policy network metadata", "network \"")
        , ("policy secret metadata", "secret \"")
        , ("policy file metadata", "file \"")
        , ("toolserver with-clause", ", with, [")
        , ("workflow state metadata", "state(:,")
        , ("route arrow surface", "] > [")
        , ("hook arrow surface", ") > [")
        ]
      suspicious =
        [ name
        | (name, snippet) <- suspiciousPatterns
        , snippet `T.isInfixOf` output
        ]
   in if not (T.isPrefixOf "// Generated by compiler-selfhost" output) || not ("export " `T.isInfixOf` output)
        then
          Left $
            singleDiagnostic
              "E_PRIMARY_COMPILER_RUNTIME"
            ...
            then Right ()
              ...
              Left $
                singleDiagnostic
                  "E_PRIMARY_COMPILER_RUNTIME"
                  "The primary Clasp compiler runtime check failed."
                  [ "hosted compile compatibility check emitted unlowered primary-surface syntax"
                  , "suspicious markers: " <> T.intercalate ", " suspicious
                  ]
...
renderHostedToolExportName :: HostedToolCommand -> HostedToolInput -> String
renderHostedToolExportName command input =
  case input of
    HostedToolInputSource _ ->
      case command of
        HostedToolCheck ->
          "checkSourceText"
        HostedToolCheckCore ->
...
hostedCompilerNativeSeedPath =
  normalise ("src" </> "stage1.native.image.json")
