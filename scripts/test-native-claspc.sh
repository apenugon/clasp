...
goal_manager_shared_cache_root="${CLASP_GOAL_MANAGER_SHARED_CACHE_PROJECT_ROOT:-$project_root}"
...
report_error() {
  ...
  case "$-" in
    *e*) printf 'test-native-claspc failed at line %s with status %s\n' "$line" "$status" >&2 ;;
  esac
  return "$status"
}
...
function artifactText(path, expected, label) {
  ...
  const text = fs.readFileSync(path, "utf8").replace(/\x1b\][^\x07]*(?:\x07|\x1b\\)/g, "");
...
native_list_dir_check="$(timeout 120 "$claspc_bin" --json check "$project_root/examples/native-list-dir/Main.clasp")"
printf '%s\n' "$native_list_dir_check" | grep -F '"status":"ok"' >/dev/null
native_list_dir_output="$(
  timeout 180 "$claspc_bin" run "$project_root/examples/native-list-dir/Main.clasp" -- "$project_root/examples/native-list-dir/fixture"
)"
...
if [[ "$native_claspc_exhaustive" != "0" ]]; then
  ...
  grep -F '[claspc-cache] native-image hit path=/tmp/clasp-nix-cache/claspc-native/native-image-cache-v5/' "$shared_cache_trace_log" >/dev/null
...
expect_command_failure_contains 'lease is owned by `manager`' env CLASP_SWARM_ACTOR=intruder "$claspc_bin" --json swarm heartbeat "$swarm_sqlite_state_root" repair
expect_command_failure_contains 'lease is owned by `manager`' env CLASP_SWARM_ACTOR=intruder "$claspc_bin" --json swarm complete "$swarm_sqlite_state_root" repair
expect_command_failure_contains 'lease is owned by `manager`' env CLASP_SWARM_ACTOR=intruder "$claspc_bin" --json swarm fail "$swarm_sqlite_state_root" repair
expect_command_failure_contains 'lease is owned by `manager`' env CLASP_SWARM_ACTOR=intruder "$claspc_bin" --json swarm release "$swarm_sqlite_state_root" repair
...
expect_command_failure_contains 'lease is owned by `manager`' env CLASP_SWARM_ACTOR=intruder "$claspc_bin" --json swarm tool "$swarm_sqlite_state_root" repair --cwd "$project_root" -- bash -lc 'printf wrong-tool > "$1"' bash "$swarm_sqlite_wrong_tool_marker"
...
swarm_sqlite_recovery_task_output="$("$claspc_bin" --json swarm task create "$swarm_sqlite_state_root" recovery-loop expired-lease --detail 'Recover stale worker lease' --lease-timeout-ms 500)"
...
printf '%s\n' "$swarm_sqlite_manager_after_repair_output" | grep -F '"latestVerifier":"native-smoke"' >/dev/null
printf '%s\n' "$swarm_sqlite_manager_after_repair_output" | grep -F '"latestVerifierStatus":""' >/dev/null
...
swarm_sqlite_denied_workspace_root="$test_root_abs/swarm-sqlite-denied-workspace"
mkdir -p "$swarm_sqlite_denied_workspace_root"
expect_command_failure_contains 'is outside allowed workspace roots' env CLASP_SWARM_ACTOR=manager "$claspc_bin" --json swarm verifier run "$swarm_sqlite_state_root" repair-2 native-smoke --cwd "$swarm_sqlite_denied_workspace_root" -- bash -lc 'printf denied-workspace > denied-workspace.marker'
[[ ! -e "$swarm_sqlite_denied_workspace_root/denied-workspace.marker" ]]
...
printf '%s\n' "$swarm_native_run_output" | grep -F '"verificationPlan":{"name":"focused-native","commands":[{"verifierName":"native-smoke"' >/dev/null
printf '%s\n' "$swarm_native_run_output" | grep -F '"verifierName":"native-regression","command":{"cwd":' >/dev/null
printf '%s\n' "$swarm_native_run_output" | grep -F '"requiredVerifierNames":["native-smoke","native-regression"]' >/dev/null
printf '%s\n' "$swarm_native_run_output" | grep -F '"verificationPlanResult":{"plan":{"name":"focused-native"' >/dev/null
printf '%s\n' "$swarm_native_run_output" | grep -F '"steps":[{"run":{"actor":"manager","command":["bash","-lc","printf verifier-ok"]' >/dev/null
printf '%s\n' "$swarm_native_run_output" | grep -F '"run":{"actor":"manager","command":["bash","-lc","printf regression-ok"]' >/dev/null
printf '%s\n' "$swarm_native_run_output" | grep -F '"passed":true,"failedVerifiers":[]' >/dev/null
printf '%s\n' "$swarm_native_run_output" | grep -F '"mailboxSummary":{"historyCount":10,"tailCount":10,"approvalCount":0,"runCount":3,"artifactCount":6' >/dev/null
printf '%s\n' "$swarm_native_run_output" | grep -F '"name":"native-smoke","role":"verifier"' >/dev/null
printf '%s\n' "$swarm_native_run_output" | grep -F '"name":"native-regression","role":"verifier"' >/dev/null
...
printf '%s\n' "$swarm_native_run_output" | grep -F '"failedVerificationPlan":{"objective":"failure-plan","task":"verification-plan-fails","plan":{"name":"focused-native-failure"' >/dev/null
printf '%s\n' "$swarm_native_run_output" | grep -F '"requiredVerifierNames":["native-plan-pass","native-plan-fail"]' >/dev/null
printf '%s\n' "$swarm_native_run_output" | grep -F '"run":{"actor":"manager","command":["bash","-lc","printf plan-pass-ok"]' >/dev/null
printf '%s\n' "$swarm_native_run_output" | grep -F '"run":{"actor":"manager","command":["bash","-lc","printf plan-fail-out; printf plan-fail-err >&2; exit 4"]' >/dev/null
printf '%s\n' "$swarm_native_run_output" | grep -F '"passed":false,"failedVerifiers":["native-plan-fail"]' >/dev/null
...
printf '%s\n' "$swarm_native_run_output" | grep -F '"failedPlanRequiredVerifiers":["native-plan-pass","native-plan-fail"],"failedPlanPassed":false,"failedPlanFailedVerifiers":["native-plan-fail"]' >/dev/null
printf '%s\n' "$swarm_native_run_output" | grep -F '"failedPlanLatestVerifier":"native-plan-fail","failedPlanLatestVerifierStatus":"failed","failedPlanMergeVerdict":"fail"' >/dev/null
...
goal_manager_ensure_stderr="$test_root_abs/ensure-goal-manager-native.stderr"
