...
const agentErgonomics = read("examples/swarm-native/AgentErgonomics.clasp");
...
assert(localAgent.includes("agentPromptHasAgentBackendPolicyRepair prompt"), "local agent should delegate backend repair detection to the shared agent ergonomics helper");
assert(agentErgonomics.includes("textIncludes prompt \"backendConfigRepair=agent-backend\""), "shared agent ergonomics helper should detect compact backend config repair markers");
assert(agentErgonomics.includes("textIncludes prompt \"policyMessages=\""), "shared agent ergonomics helper should require all backend policy messages before claiming repair-context coverage");
...
assert(localPlanner.includes("agentPromptHasPlannerBackendPolicyRepair prompt"), "local planner should delegate backend repair detection to the shared agent ergonomics helper");
assert(agentErgonomics.includes("textIncludes prompt \"plannerBackendConfigRepair=agent-backend\""), "shared agent ergonomics helper should detect compact planner backend config repair markers");
assert(agentErgonomics.includes("textIncludes prompt \"backendConfigRepair=agent-backend\""), "shared agent ergonomics helper should detect compact loop backend config repair markers");
...
assert(managerTypes.includes("nextAction : Str"), "GoalManager status should expose the manager next action");
assert(managerTypes.includes("autonomyBlocked : Bool"), "GoalManager status should expose whether autonomy is blocked");
assert(managerTypes.includes("autonomyBlockers : [Str]"), "GoalManager status should expose autonomy blocker categories");
...
assert(managerStateView.includes("nextAction = statusNextAction"), "GoalManager status view should compute the next manager action");
assert(managerStateView.includes("autonomyBlocked = statusAutonomyBlocked"), "GoalManager status view should compute autonomy blockage");
assert(managerStateView.includes("autonomyBlockers = autonomyBlockersFor waitingReasons"), "GoalManager status view should expose autonomy blocker categories");
...
assert(managerReports.includes("nextAction = statusNextAction"), "GoalManager manager reports should compute the next manager action");
assert(managerReports.includes("autonomyBlocked = statusAutonomyBlocked"), "GoalManager manager reports should compute autonomy blockage");
assert(managerReports.includes("autonomyBlockers = autonomyBlockersFor waitingReasons"), "GoalManager manager reports should expose autonomy blocker categories");
