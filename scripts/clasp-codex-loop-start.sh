...
min_available_memory_mb="${CLASP_CODEX_LOOP_MIN_AVAILABLE_MEMORY_MB:-45056}"
min_available_disk_mb="${CLASP_CODEX_LOOP_MIN_AVAILABLE_DISK_MB:-16384}"
min_disk_headroom_mb="${CLASP_CODEX_LOOP_MIN_DISK_HEADROOM_MB:-${CLASP_GENERATED_STATE_MIN_HEADROOM_MB:-1024}}"
...
validate_non_negative_integer "CLASP_CODEX_LOOP_MIN_AVAILABLE_DISK_MB" "$min_available_disk_mb"
validate_non_negative_integer "CLASP_CODEX_LOOP_MIN_DISK_HEADROOM_MB" "$min_disk_headroom_mb"
...
job_dir="$(
  "${managed_job_args[@]}" \
    -- bash -c '
      ...
      "$@" >>"$log_file" 2>&1
    ' \
      managed-codex-loop "$log_file" "$pid_file" \
      env \
...
if clasp_swarm_wait_managed_job_immediate_terminal_status "$job_dir"; then
  clasp_swarm_print_managed_job_terminal_report "codex loop" "$job_dir"
