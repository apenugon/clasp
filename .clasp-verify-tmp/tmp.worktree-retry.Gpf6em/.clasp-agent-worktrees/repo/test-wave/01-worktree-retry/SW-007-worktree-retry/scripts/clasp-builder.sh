...
{"summary":"builder finished for $task_id","files_touched":[],"tests_run":[],"residual_risks":[]}
JSON
  exit 0
fi
printf 'recovered-builder-change\n' > "$workspace/feature.txt"
cat > "$report_json" <<JSON
{"summary":"builder finished for $task_id","files_touched":["feature.txt"],"tests_run":[],"residual_risks":[]}
JSON
