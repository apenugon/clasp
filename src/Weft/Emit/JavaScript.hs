...
emitRuntimePrelude =
  [ ""
  , "function $weftRuntime(name) {"
  , "  const runtime = globalThis.__weftRuntime;"
  , "  if (!runtime || !(name in runtime)) {"
  , "    throw new Error(`Missing Weft runtime binding: ${name}`);"
  , "  }"
  , "  return runtime[name];"
  , "}"
  , ""
  , "function $weftExpectObject(value, path) {"
  ...
  , "}"
  , ""
  , "function $weftExpectInt(value, path) {"
  ...
  , "}"
  , ""
  , "function $weftExpectStr(value, path) {"
  ...
  , "}"
  , ""
  , "function $weftExpectBool(value, path) {"
  ...
  , "}"
  , ""
  , "function $weftRequireField(objectValue, fieldName, path) {"
...
emitPrimitiveCodecHelpers =
  [ "function $decode_Int(jsonText) { return $weftExpectInt(JSON.parse(jsonText), \"value\"); }"
  , "function $encode_Int(value) { return JSON.stringify($weftExpectInt(value, \"value\")); }"
  , "function $decode_Str(jsonText) { return $weftExpectStr(JSON.parse(jsonText), \"value\"); }"
  , "function $encode_Str(value) { return JSON.stringify($weftExpectStr(value, \"value\")); }"
  , "function $decode_Bool(jsonText) { return $weftExpectBool(JSON.parse(jsonText), \"value\"); }"
  , "function $encode_Bool(value) { return JSON.stringify($weftExpectBool(value, \"value\")); }"
  , ""
  ]
...
emitRecordCodecHelpers recordDecl =
  ...
  , "  const objectValue = $weftExpectObject(value, path);"
  ]
    ...
    <> [ "  return { " <> T.intercalate ", " (fmap (\fieldDecl -> recordFieldDeclName fieldDecl <> ": " <> recordFieldDeclName fieldDecl) (recordDeclFields recordDecl)) <> " };"
       ...
       , "function " <> encodeName <> "(value) { return JSON.stringify(" <> serializeName <> "(" <> validateName <> "(value, \"value\"))); }"
       , ""
       ]
  where
    ...
    emitValidateField fieldDecl =
      let fieldName = recordFieldDeclName fieldDecl
       ...
       in [ "  const " <> fieldName <> "Value = $weftRequireField(objectValue, " <> emitStringLiteral fieldName <> ", path);"
...
emitValidator typ valueRef pathRef =
  case typ of
    TInt ->
      "$weftExpectInt(" <> valueRef <> ", " <> pathRef <> ")"
    TStr ->
      "$weftExpectStr(" <> valueRef <> ", " <> pathRef <> ")"
    TBool ->
      "$weftExpectBool(" <> valueRef <> ", " <> pathRef <> ")"
    TNamed name ->
...
emitSerializer typ valueRef =
  case typ of
    TInt ->
      "$weftExpectInt(" <> valueRef <> ", \"value\")"
    TStr ->
      "$weftExpectStr(" <> valueRef <> ", \"value\")"
    TBool ->
      "$weftExpectBool(" <> valueRef <> ", \"value\")"
    TNamed name ->
