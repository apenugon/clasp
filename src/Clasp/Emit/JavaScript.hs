...
emitRuntimePrelude =
  ...
  , "    }"
  , "    try {"
  , "      const reduced = $claspWorkflowReduceResult(handler(normalizedRun.state, normalizedMessage.payload, normalizedMessage, Object.freeze({"
  , "        attempt,"
...
renderWorkflowDoc workflowDecl =
  T.unlines
    ...
    , "- Start: `workflow.start(snapshot, { deadlineAt?, retry? }) -> { checkpoint, state, deliveries, processedIds, deadlineAt, cancelled, retryPolicy }`"
    ...
    , "- Deliver: `workflow.deliver(run, { id, payload }, reducer, { now?, retry? })` suppresses duplicates, enforces cancellation and deadlines, and retries reducer failures with bounded backoff"
