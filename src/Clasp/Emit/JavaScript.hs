...
emitRuntimePrelude =
  ...
  , "}"
  , ""
  , "function $claspWorkflowStart(workflowName, snapshot, decode, options = null) {"
  ...
  , "}"
  , ""
  , "function $claspWorkflowDeliver(workflowName, run, message, handler, encode, replayed = false, options = null) {"
  ...
  , "}"
  , ""
  , "function $claspWorkflowProcessNext(workflowName, run, handler, encode, options = null) {"
  , "  const normalizedRun = $claspWorkflowNormalizeRun(run, workflowName);"
  ...
  , "  });"
  , "  const delivery = $claspWorkflowDeliver(workflowName, queuedRun, nextMessage, handler, encode, false, options);"
  ...
  , "}"
  , ""
  , "function $claspWorkflowDrainMailbox(workflowName, run, handler, encode, options = null) {"
  , "  let currentRun = $claspWorkflowNormalizeRun(run, workflowName);"
  , "  const deliveries = [];"
  , "  while (currentRun.mailbox.length > 0) {"
  , "    const processed = $claspWorkflowProcessNext(workflowName, currentRun, handler, encode, options);"
  ...
  , "}"
  , ""
  , "function $claspWorkflowReplay(workflowName, snapshot, messages, handler, decode, encode, options = null) {"
  , "  if (!Array.isArray(messages)) {"
  , "    throw new Error(`Workflow ${workflowName} replay requires an array of messages.`);"
  , "  }"
  , "  let run = $claspWorkflowStart(workflowName, snapshot, decode, options);"
  , "  for (const message of messages) {"
  , "    const delivery = $claspWorkflowDeliver(workflowName, run, message, handler, encode, true, options);"
  , "    run = delivery.run;"
...
emitControlPlaneExports modl =
  ...
  where
    ...
    emitWorkflow workflowDecl =
      ...
      , "    start(snapshot, options) { return $claspWorkflowStart(" <> emitStringLiteral (workflowDeclName workflowDecl) <> ", snapshot, $decode_" <> codecSuffix (workflowDeclStateType workflowDecl) <> ", options); },"
      ...
      , "    processNext(run, handler, options) { return $claspWorkflowProcessNext(" <> emitStringLiteral (workflowDeclName workflowDecl) <> ", run, handler, $encode_" <> codecSuffix (workflowDeclStateType workflowDecl) <> ", options); },"
      , "    drainMailbox(run, handler, options) { return $claspWorkflowDrainMailbox(" <> emitStringLiteral (workflowDeclName workflowDecl) <> ", run, handler, $encode_" <> codecSuffix (workflowDeclStateType workflowDecl) <> ", options); },"
      , "    deliver(run, message, handler, options) { return $claspWorkflowDeliver(" <> emitStringLiteral (workflowDeclName workflowDecl) <> ", run, message, handler, $encode_" <> codecSuffix (workflowDeclStateType workflowDecl) <> ", false, options); },"
      , "    replay(snapshot, messages, handler, options) { return $claspWorkflowReplay(" <> emitStringLiteral (workflowDeclName workflowDecl) <> ", snapshot, messages, handler, $decode_" <> codecSuffix (workflowDeclStateType workflowDecl) <> ", $encode_" <> codecSuffix (workflowDeclStateType workflowDecl) <> ", options); },"
