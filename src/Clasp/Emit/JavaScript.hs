...
emitRuntimePrelude =
  ...
  , "}"
  , ""
  , "function $claspWorkflowStart(workflowName, snapshot, decode) {"
  ...
  , "}"
  , ""
  , "function $claspWorkflowDeliver(workflowName, run, message, handler, encode, replayed = false) {"
  ...
  , "    });"
  , "  }"
  , "  const reduced = $claspWorkflowReduceResult(handler(normalizedRun.state, normalizedMessage.payload, normalizedMessage));"
  ...
  , "}"
  , ""
  , "function $claspWorkflowReplay(workflowName, snapshot, messages, handler, decode, encode) {"
  , "  if (!Array.isArray(messages)) {"
  , "    throw new Error(`Workflow ${workflowName} replay requires an array of messages.`);"
  , "  }"
  , "  let run = $claspWorkflowStart(workflowName, snapshot, decode);"
  , "  for (const message of messages) {"
  , "    run = $claspWorkflowDeliver(workflowName, run, message, handler, encode, true).run;"
  , "  }"
  , "  return run;"
  , "}"
  , ""
...
emitControlPlaneExports modl =
  ...
  where
    ...
    emitWorkflow workflowDecl =
      ...
      , "    start(snapshot) { return $claspWorkflowStart(" <> emitStringLiteral (workflowDeclName workflowDecl) <> ", snapshot, $decode_" <> codecSuffix (workflowDeclStateType workflowDecl) <> "); },"
      , "    deliver(run, message, handler) { return $claspWorkflowDeliver(" <> emitStringLiteral (workflowDeclName workflowDecl) <> ", run, message, handler, $encode_" <> codecSuffix (workflowDeclStateType workflowDecl) <> "); },"
      , "    replay(snapshot, messages, handler) { return $claspWorkflowReplay(" <> emitStringLiteral (workflowDeclName workflowDecl) <> ", snapshot, messages, handler, $decode_" <> codecSuffix (workflowDeclStateType workflowDecl) <> ", $encode_" <> codecSuffix (workflowDeclStateType workflowDecl) <> "); }"
      , "  },"
      ]
...
renderWorkflowDoc workflowDecl =
  T.unlines
    ...
    , "- Start: `workflow.start(snapshot) -> { checkpoint, state, deliveries, processedIds }`"
    , "- Deliver: `workflow.deliver(run, { id, payload }, reducer)` applies a message once and suppresses duplicate ids"
    , "- Replay: `workflow.replay(snapshot, messages, reducer)` deterministically rebuilds message-driven state"
    ]
