...
emitRuntimePrelude =
  ...
  , "  return Object.freeze({"
  , "    ...normalizedRun,"
  , "    deadlineAt: $claspWorkflowNormalizeDeadline(deadlineAt, `${workflowName}.deadlineAt`)"
  , "  });"
  , "}"
  , ""
  ...
  , "    ...normalizedRun,"
  , "    cancelled: true,"
  , "    cancelReason: typeof reason === \"string\" && reason !== \"\" ? reason : \"cancelled\""
  , "  });"
  , "}"
  , ""
  ...
  , "    delivery,"
  , "    run: consumed ? delivery.run : normalizedRun"
  , "  });"
  , "}"
  , ""
...
renderWorkflowDoc workflowDecl =
  T.unlines
    ...
    , "- Start: `workflow.start(snapshot, { deadlineAt?, retry?, supervisor?, mailbox? }) -> { checkpoint, state, deliveries, processedIds, mailbox, deadlineAt, cancelled, retryPolicy, supervision }`"
    ...
    , "- Replay: `workflow.replay(snapshot, messages, reducer, options?)` deterministically rebuilds message-driven state until cancellation, deadline, or retry exhaustion"
