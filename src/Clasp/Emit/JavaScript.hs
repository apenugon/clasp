...
emitRuntimePrelude =
  ...
  , "  }"
  , "  return Object.freeze({"
  , "    now: rawOptions.now === undefined ? Date.now() : $claspWorkflowNormalizeDeadline(rawOptions.now, `${workflowName}.delivery.now`),"
...
renderWorkflowDoc workflowDecl =
  T.unlines
    ...
    , "- Deliver: `workflow.deliver(run, { id, payload }, reducer, { now?, retry?, degradedHandler? })` suppresses duplicates, enforces cancellation and deadlines, retries reducer failures with bounded backoff, and allows supervised degraded-mode fallbacks"
