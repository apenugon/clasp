...
- Implement `TY-022` as one narrow slice of work: Add semantic proof and result caching keyed by graph identity, compiler version, and relevant world or environment state so already-discharged reasoning does not repeat unnecessarily
...
- `TY-022` is implemented without breaking previously integrated tasks
