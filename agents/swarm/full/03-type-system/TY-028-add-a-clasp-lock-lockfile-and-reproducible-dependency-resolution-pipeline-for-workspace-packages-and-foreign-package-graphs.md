...
Add a `clasp.lock` lockfile and reproducible dependency-resolution pipeline for workspace packages and foreign package graphs
...
- Implement `TY-028` as one narrow slice of work: Add a `clasp.lock` lockfile and reproducible dependency-resolution pipeline for workspace packages and foreign package graphs
...
- `TY-028` is implemented without breaking previously integrated tasks
