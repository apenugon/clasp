...
fn push_project_module(
    ...
    bundled_sources.push(source.clone());
...
fn main() -> ExitCode {
    ...
    let exit_code = (|| unsafe {
        ...
        if fs::write(output_path, string_bytes(dispatch_value as *mut ClaspRtString)).is_err() {
            return fail("failed to write hosted compiler result");
        }
