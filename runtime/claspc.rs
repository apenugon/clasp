...
mod tests {
    use super::{
        augment_compiled_js_unsafe_quarantine, collect_project_module_postorder,
        conservative_module_interface_fingerprint, count_decl_names,
        embedded_final_status_path, empty_rhs_parse_diagnostic_for_source,
        final_status_output_bytes, form_body_to_json, harden_native_image_runtime_bindings,
        json_value_string, json_value_string_array, merge_json_arrays, native_image_cache_dir,
    ...
    #[test]
    fn compiled_js_unsafe_quarantine_replaces_identity_codecs_and_strips_parenthesized_types() {
        let emitted = br#"// Generated by compiler-selfhost
function $claspRuntime(name) { return globalThis.__claspRuntime[name]; }
export const __claspHostBindings = [
  {
    name: "loadBox",
    runtimeName: "loadBox",
    params: [{ type: "Str", fromHost(value, path = "value") { return value; }, toHost(value, path = "value") { return value; } }],
    returns: { type: "Option (Box Str)", fromHost(value, path = "result") { return value; }, toHost(value, path = "result") { return value; } }
  },
];
"#;

        let augmented = String::from_utf8(augment_compiled_js_unsafe_quarantine(emitted.to_vec()))
            .expect("augmented JS should remain UTF-8");

        assert!(
            augmented.contains("function $claspParseHostTypeDescriptor(rawType)"),
            "expected host type descriptor parser"
        );
        assert!(
            augmented.contains("function $claspStripBalancedOuterParens(text)"),
            "expected balanced outer parenthesis stripping helper"
        );
        assert!(
            augmented.contains("$claspCreateHostTypeCodec(\"Option (Box Str)\", \"result\")"),
            "expected parenthesized nested return codec replacement"
        );
        assert!(
            augmented.contains("descriptor.name === \"Result\""),
            "expected Result values to be refined instead of quarantined"
        );
        assert!(
            !augmented.contains("returns: { type: \"Option (Box Str)\", fromHost(value, path = \"result\") { return value; }"),
            "identity return codec should be replaced"
        );
    }
...
fn host_type_quarantine_prelude() -> &'static str {
    r#"const $claspUnsafeQuarantineBrand = Symbol.for("clasp.unsafeQuarantine");

function $claspIsUnsafeQuarantined(value) {
  return Boolean(value && typeof value === "object" && value[$claspUnsafeQuarantineBrand] === true);
}

function $claspRenderHostTypeDescriptor(descriptor) {
  if (!descriptor || typeof descriptor !== "object") {
    return "Unknown";
  }
  if (typeof descriptor.raw === "string" && descriptor.raw !== "") {
    return descriptor.raw;
  }
  if (descriptor.kind === "list") {
    return `[${$claspRenderHostTypeDescriptor(descriptor.item)}]`;
  }
  if (descriptor.kind === "apply") {
    return `${descriptor.name} ${descriptor.args.map($claspRenderHostTypeDescriptor).join(" ")}`;
  }
  return String(descriptor.name ?? "Unknown");
}

function $claspQuarantineHostValue(value, descriptor, path, reason = "foreign-trusted") {
  if ($claspIsUnsafeQuarantined(value)) {
    return value;
  }
  return Object.freeze({
    kind: "clasp-unsafe-quarantine",
    tainted: true,
    taint: "foreign-trusted",
    reason,
    path,
    type: $claspRenderHostTypeDescriptor(descriptor),
    value,
    [$claspUnsafeQuarantineBrand]: true
  });
}

function $claspHasBalancedOuterParens(text) {
...
function $claspStripBalancedOuterParens(text) {
  let current = String(text ?? "").trim();
  while ($claspHasBalancedOuterParens(current)) {
    current = current.slice(1, -1).trim();
  }
  return current;
}

function $claspUnwrapHostTypeDelimiter(text, open, close) {
  ...
  return depth === 0 ? current.slice(1, -1).trim() : null;
}

function $claspSplitHostTypeAtoms(text) {
  const atoms = [];
  let current = "";
  let parenDepth = 0;
  let bracketDepth = 0;
...
function $claspParseHostTypeDescriptor(rawType) {
  const typeText = $claspStripBalancedOuterParens(rawType);
  const listInner = $claspUnwrapHostTypeDelimiter(typeText, "[", "]");
  if (listInner !== null) {
    return Object.freeze({ kind: "list", raw: typeText, item: $claspParseHostTypeDescriptor(listInner) });
  }
  const atoms = $claspSplitHostTypeAtoms(typeText);
  if (atoms.length === 0) {
    return Object.freeze({ kind: "unknown", raw: typeText, name: "Unknown" });
  }
  if (atoms.length > 1) {
    return Object.freeze({
      kind: "apply",
      raw: typeText,
      name: atoms[0],
      args: Object.freeze(atoms.slice(1).map((atom) => $claspParseHostTypeDescriptor(atom)))
    });
  }
  return Object.freeze({ kind: "name", raw: typeText, name: atoms[0] });
}

function $claspHostTypeIsDynamic(descriptor) {
  if (!descriptor || typeof descriptor !== "object") {
    return true;
  }
  ...
  if (descriptor.kind === "name") {
    ...
    return name === "" || name === "Dynamic" || name === "Unknown" || name === "Any" || name === "any" || /^[a-z]/.test(name);
  }
  return false;
}

function $claspPrepareStructuredHostValue(descriptor, value, path) {
  if (descriptor?.kind === "name" && descriptor.name === "Str") {
    return value;
  }
  if (typeof value !== "string") {
    return value;
  ...
  try {
    ...
    throw new Error(`${path} expected valid JSON text for ${$claspRenderHostTypeDescriptor(descriptor)}`);
  }
}

function $claspRefineHostValue(descriptor, value, path = "value") {
  const rawValue = $claspIsUnsafeQuarantined(value) ? value.value : value;
  if ($claspHostTypeIsDynamic(descriptor)) {
    return $claspQuarantineHostValue(rawValue, descriptor, path, "unresolved-host-type");
  }
  const prepared = $claspPrepareStructuredHostValue(descriptor, rawValue, path);
  if (descriptor.kind === "list") {
    return Object.freeze($claspExpectArray(prepared, path).map((item, index) => $claspRefineHostValue(descriptor.item, item, `${path}[${index}]`)));
  }
  if (descriptor.kind === "name") {
    if (descriptor.name === "Str") {
      return $claspExpectString(prepared, path);
    }
    if (descriptor.name === "Int") {
      return $claspExpectInt(prepared, path);
    }
    if (descriptor.name === "Bool") {
      return $claspExpectBool(prepared, path);
    }
    const schema = typeof __claspSchemas !== "undefined" ? (__claspSchemas[descriptor.name] ?? null) : null;
    if (schema && typeof schema.fromHost === "function") {
      return schema.fromHost(prepared, path);
    }
    return $claspQuarantineHostValue(prepared, descriptor, path, "unresolved-host-type");
  }
  if (descriptor.kind === "apply") {
    if (descriptor.name === "Option" && descriptor.args.length === 1) {
      const variant = $claspExpectArray(prepared, path);
      const tag = $claspExpectString(variant[0], `${path}.$tag`);
      if (tag === "None" && variant.length === 1) {
        return Object.freeze(["None"]);
      }
      if (tag === "Some" && variant.length === 2) {
        return Object.freeze(["Some", $claspRefineHostValue(descriptor.args[0], variant[1], `${path}.Some`)]);
      }
      throw new Error(`${path} expected Option value`);
    }
    if (descriptor.name === "Result" && descriptor.args.length === 1) {
      const variant = $claspExpectArray(prepared, path);
      const tag = $claspExpectString(variant[0], `${path}.$tag`);
      if (tag === "Ok" && variant.length === 2) {
        return Object.freeze(["Ok", $claspRefineHostValue(descriptor.args[0], variant[1], `${path}.Ok`)]);
      }
      if (tag === "Err" && variant.length === 2) {
        return Object.freeze(["Err", $claspExpectString(variant[1], `${path}.Err`)]);
      }
      throw new Error(`${path} expected Result value`);
    }
    const schema = typeof __claspSchemas !== "undefined" ? (__claspSchemas[descriptor.name] ?? null) : null;
    if (schema && typeof schema.fromHost === "function") {
      const objectValue = schema.fromHost(prepared, path);
      if (descriptor.args.length === 1 && objectValue && typeof objectValue === "object" && !Array.isArray(objectValue) && Object.prototype.hasOwnProperty.call(objectValue, "value")) {
        return Object.freeze({ ...objectValue, value: $claspRefineHostValue(descriptor.args[0], objectValue.value, `${path}.value`) });
      }
      return objectValue;
    }
    return $claspQuarantineHostValue(prepared, descriptor, path, "unresolved-host-type");
...
function $claspCreateHostTypeCodec(rawType, defaultPath) {
  const descriptor = $claspParseHostTypeDescriptor(rawType);
  return Object.freeze({
    type: rawType,
    descriptor,
    fromHost(value, path = defaultPath) { return $claspRefineHostValue(descriptor, value, path); },
    toHost(value, path = defaultPath) { return $claspRefineHostValue(descriptor, value, path); }
  });
}
...
fn find_json_string_end(text: &str, quote_index: usize) -> Option<usize> {
    let mut escaped = false;
    for (offset, ch) in text[quote_index + 1..].char_indices() {
        if escaped {
            escaped = false;
            continue;
        }
...
fn replace_identity_host_codecs(output_text: &str) -> String {
    let needle = "{ type: ";
    let value_suffix =
        ", fromHost(value, path = \"value\") { return value; }, toHost(value, path = \"value\") { return value; } }";
    let result_suffix =
        ", fromHost(value, path = \"result\") { return value; }, toHost(value, path = \"result\") { return value; } }";
    let mut rendered = String::with_capacity(output_text.len());
    let mut cursor = 0;

    while let Some(relative_index) = output_text[cursor..].find(needle) {
        let start = cursor + relative_index;
        let quote_index = start + needle.len();
        if !output_text[quote_index..].starts_with('"') {
            rendered.push_str(&output_text[cursor..start + needle.len()]);
            cursor = start + needle.len();
            continue;
        }
        let Some(string_end) = find_json_string_end(output_text, quote_index) else {
            rendered.push_str(&output_text[cursor..start + needle.len()]);
            cursor = start + needle.len();
            continue;
        };
        let after_string = string_end + 1;
        let rest = &output_text[after_string..];
        let (default_path, suffix) = if rest.starts_with(value_suffix) {
            ("value", value_suffix)
        } else if rest.starts_with(result_suffix) {
            ("result", result_suffix)
        } else {
            rendered.push_str(&output_text[cursor..after_string]);
            cursor = after_string;
            continue;
        };
        rendered.push_str(&output_text[cursor..start]);
        rendered.push_str("$claspCreateHostTypeCodec(");
        rendered.push_str(&output_text[quote_index..after_string]);
        rendered.push_str(", ");
        rendered.push_str(&json_string(default_path));
        rendered.push(')');
        cursor = after_string + suffix.len();
    }

    rendered.push_str(&output_text[cursor..]);
    rendered
}

fn augment_compiled_js_unsafe_quarantine(output_bytes: Vec<u8>) -> Vec<u8> {
    let output_text = match String::from_utf8(output_bytes) {
        Ok(output_text) => output_text,
        Err(error) => return error.into_bytes(),
    };
    if !output_text.contains("export const __claspHostBindings = [")
        || output_text.contains("$claspParseHostTypeDescriptor")
    {
        return output_text.into_bytes();
    }

    let replaced = replace_identity_host_codecs(&output_text);
    if replaced == output_text {
        return output_text.into_bytes();
    }

    let marker = "function $claspRuntime(name) {";
    if let Some(index) = replaced.find(marker) {
        let mut augmented = String::with_capacity(replaced.len() + host_type_quarantine_prelude().len());
        augmented.push_str(&replaced[..index]);
        augmented.push_str(host_type_quarantine_prelude());
        augmented.push_str(&replaced[index..]);
        augmented.into_bytes()
    } else {
        format!("{}{}", host_type_quarantine_prelude(), replaced).into_bytes()
    }
}
...
fn run_build(
    ...
    let output_bytes = if matches!(options.command, Command::Compile) && target_name == "frontend-js" {
        augment_compiled_js_unsafe_quarantine(output_bytes)
    } else {
        output_bytes
    };
