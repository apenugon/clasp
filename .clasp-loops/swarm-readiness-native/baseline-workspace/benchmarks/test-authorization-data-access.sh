...
workspace_root="$project_root/benchmarks/workspaces/authorization-data-access-check"

rm -rf "$workspace_root"
mkdir -p "$workspace_root"

verify_clasp_solution() {
  local task_id="clasp-authorization-data-access"
...
verify_typescript_solution() {
  local task_id="ts-authorization-data-access"
  local workspace="$workspace_root/$task_id"
  local solution_root="$project_root/benchmarks/tasks/$task_id/solution"
