...
if [[ -z "$candidate_arg" ]]; then
  echo "usage: bash $eval_root/validate.sh <candidate-dir>" >&2
  exit 2
fi
...
output_root="$(mktemp -d "${TMPDIR:-/tmp}/clasp-eval-lead-digest.XXXXXX")"
...
node "$eval_root/validate.mjs" "$candidate_dir" "$compiled_path" "$context_path" "$air_path"
