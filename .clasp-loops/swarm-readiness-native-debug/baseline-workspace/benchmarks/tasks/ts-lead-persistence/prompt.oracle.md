...
- The exact app-owned edit surfaces for this TypeScript variant are `src/server/main.ts`, `src/server/store.ts`, `src/server/runtime-modules.d.ts`, and `src/server/dev.ts`.
