...
nix develop "$project_root" --command bash -lc "
  set -euo pipefail
  cd \"$project_root\"
  cabal run claspc -- compile \"$workspace_root/Main.clasp\" -o \"$workspace_root/build/Main.js\"
  cabal run claspc -- compile \"$workspace_root/Main.next.clasp\" -o \"$workspace_root/build/Main.next.js\"
"
